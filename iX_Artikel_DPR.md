...
<!-- $\sigma(z) _ i = \frac{e^{z _ i}}{\sum _ {j=1}^{\left|K\right|} e^{z _ j}}$ --> <img color-scheme="light" style="transform: translateY(0.1em); background: white;" src="svg\u20xjHJjAM.svg">
...
<!-- $\sigma(z) _ i = \frac{e^{\text{sim}(q _ i,p _ {i}^+)}}{e^{\text{sim}(q _ i,p _ {i}^+)} + \sum _ {j=1}^n{e^{\text{sim}(q _ i,p _ {i,j}^-)}}}$ --> <img color-scheme="light" style="transform: translateY(0.1em); background: white;" src="svg\WrSYgkwGCR.svg">
...
<!-- $L(q _ i, p{ _ i}^+, p _ {i, 1}^-, ..., p _ {i, n}^-)=-\log(\sigma(z) _ i)=-\log\frac{e^{\text{sim}(q _ i,p _ {i}^+)}}{e^{\text{sim}(q _ i,p _ {i}^+)} + \sum _ {j=1}^n{e^{\text{sim}(q _ i,p _ {i,j}^-)}}}$ --> <img color-scheme="light" style="transform: translateY(0.1em); background: white;" src="svg\tPImr85tkf.svg">
